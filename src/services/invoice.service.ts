...
class InvoiceService {
    async generateInvoice(sale_id: string, reply: FastifyReply) {

        const sale = await Sale.findOne({
            where: { id: sale_id }, include: [{ model: SaleItem, as: "items", include: [{ model: Medicine, as: "medicine", attributes: ["name", "company"] }] }]
        });
